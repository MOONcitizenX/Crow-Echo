...
export class BirdInfoCard extends BaseElement {
	constructor(langState, quizState, baseClass, withDefault) {
		...
		this.updateState = (state) => {
			...
			if (currentBird) {
				...
				this.birdDesc.elem.textContent =
                    currentBird[`description${this.lang.get() === 'ru' ? '' : '_en'}`];
		...
		this.updateLang = (val) => {
			const currentBird = birdsData[this.quizState.get().quizLvl][this.quizState.get().clickedCard];
			if (currentBird) {
				this.birdName.elem.textContent =
		...
		if (withDefault) {
			...
			this.defaultMsgChildren = textContent[this.lang.get()].default_msg.map((text) => new BaseElement({
				tag: 'p',
	...
	render() {
		...
		this.player.render();
		this.addChildren(this.defaultMsg, this.birdImg, this.birdNameContainer, this.player, this.birdDesc);
